...
object DataType {
    ...
    def main(args: Array[String]): Unit = {
        ...
        // 对象恒等比较
        // 如果需要比较两个对象是否相等，可以使用==或者！=操作符
        // 在scala中“==”比较的是两个对象的值是否相等，而在java中“==”比较的是两个对象的地址是否相等
        // 而对于引用类型的比较使用另外的操作符eq和ne
        println("两个对象相比较" + (1 == 2))
        println("两个对象相比较" + (1 != 2))
        println("两个集合相比较" + (List(1,2,3) == List(1,2,3)))
        println("字符串相比较" + (("he" + "llo") == "hello"))
        // 对于eq和ne而言，eq()相当于java中的equals()方法 不忽略大小写，ne()相当于java中的equalsIgnoreCase()
        println(str.eq(s))
        println(str.ne(s))

        // 操作符的优先级和左右结合性
        // Scala 的操作符的优先级和 Java 基本相同，如果有困惑时，可以使用（）改变操作符的优先级。
